...
float max_10(float* vec,float length)
{
...
int main()
{
    ...
    float amean2[16] = {0.0171751,0.218673,-3.65938e-08,-3.65467e-12,0.023393,-3.93884e-16,-1.8007e-18,-3.09291e-15,-1.19799e-09,0.0574816,-2.58292e-08,0.0781449,-3.09892e-14,-2.13678e-13,0.180576,-1.09185e-12 };
    float avariance2[16] = {0.000282618,0.00894631,1.36869e-17,3.10376e-21,0.000574641,3.41789e-27,6.63849e-33,1.54395e-24,3.9622e-23,0.00624138,2.37728e-26,0.00461732,1.98225e-23,5.02278e-21,0.0185279,2.12726e-21 };
    float aaffine_r2[16] = {0.141512,0.316052,-8.67964e-42,-1.03136e-42,0.138144,-2.39482e-42,2.77597e-42,-2.9161e-42,-2.89228e-42,0.264392,4.13011e-06,0.23389,2.08179e-14,3.37713e-42,0.190538,-8.55773e-42 };
    float aaffine_c2[16] = {0.0439228,0.0946699,4.76021e-42,-6.79349e-42,-0.0401458,-2.96515e-42,5.2829e-42,6.46139e-42,-5.13856e-42,0.163904,6.97939e-13,0.119168,-5.67453e-17,6.97847e-43,0.40046,9.12245e-43 };


    float amean3[8] = {-0.0632374,-2.38747e-09,0.115511,0.092088,-0.169158,0.104168,-0.135601,0.0550785 };
    float avariance3[8] =  {0.00500648,4.96827e-28,0.00285935,0.00991006,0.00554325,0.00595959,0.00522964,0.0054756 };
    float aaffine_r3[8] = {1.12644,2.48618e-41,1.34908,1.15442,1.33413,1.2364,1.56656,1.19202 };
    float aaffine_c3[8] = {0.802881,-3.59344e-33,0.69274,0.89005,0.850165,0.747865,0.728998,0.949102 };




    float abias_fc2[16] = {-9.05632e-05,5.17794e-05,9.7691e-07,-1.5289e-15,-2.39265e-05,-1.22344e-14,2.67102e-17,3.06051e-15,4.55131e-08,-7.44568e-05,5.06537e-07,-6.25097e-05,9.07402e-20,-2.85163e-15,2.21367e-05,-3.60706e-19 };
    float abias_fc3[8] = {-7.93103e-05,3.2407e-08,-5.82372e-06,-2.68044e-05,3.87592e-05,-1.72586e-05,3.61977e-05,-2.6987e-06 };
    float abias_fc4[4] = {-0.0603522,0.0478874,0.279656,-0.342277 };



    float aweight_fc2[16][32] = {{0.0258486,0.00855987,0.0365778,-0.0570332,-0.00571106,1.17289e-42,-0.0410533,2.94207e-06,7.80034e-05,0.0115584,0.00086485,0.0232137,-0.00708458,-0.013059,-0.0136971,5.54214e-42,-4.62835e-18,0.0111777,0.104591,-1.28164e-07,7.13969e-15,0.010956,1.1057e-05,0.0498366,2.53443e-06,0.0233057,-2.11542e-25,0.00998965,-7.06254e-42,-2.66626e-16,0.0628843,2.03194e-13 },
    ...
    float aweight_fc3[8][16] = {{0.0373114,-0.14218,2.8913e-41,5.77335e-43,-0.00112834,-1.73621e-42,-3.54388e-42,-8.28448e-42,-1.86092e-42,-0.186936,-2.45041e-13,-0.181806,4.61976e-21,-2.68209e-42,0.0685517,-5.16238e-42 },
    ...
    float aweight_fc4[4][8] = {{-0.140569,1.54272e-33,0.179656,0.743521,0.150634,0.880231,-0.761842,-1.19109 },
    ...
    weight_fc2 = (float**) malloc(16*sizeof(float*));
	for(i=0;i<16;i=i+1)
	{
		weight_fc2[i] = (float*) malloc(32*sizeof(float));
		for(j=0;j<32;j=j+1)
		{
			weight_fc2[i][j] = aweight_fc2[i][j];
   	...
   	bias_fc2 = (float*) malloc(16*sizeof(float));

	for(i=0;i<16;i=i+1)
	{
		bias_fc2[i] = 	abias_fc2[i];

    }
    
    weight_fc3 = (float**) malloc(8*sizeof(float*));
	for(i=0;i<8;i=i+1)
	{
		weight_fc3[i] = (float*) malloc(16*sizeof(float));
		for(j=0;j<16;j=j+1)
		{
			weight_fc3[i][j] = aweight_fc3[i][j];
			//weight_fc3[i][j] = 1;
   	...
   	bias_fc3 = (float*) malloc(8*sizeof(float));
   	
	for(i=0;i<8;i=i+1)
	{
		bias_fc3[i] = abias_fc3[i];
		//bias_fc3[i] = 45;
		
    }   
    
    weight_fc4 = (float**) malloc(4*sizeof(float*));
	for(i=0;i<4;i=i+1)
	{
		weight_fc4[i] = (float*) malloc(8*sizeof(float));
		for(j=0;j<8;j=j+1)
		{
			weight_fc4[i][j] = aweight_fc4[i][j];
			//weight_fc4[i][j] =7;
   	...
   	bias_fc4 = (float*) malloc(4*sizeof(float));
   	
	for(i=0;i<4;i=i+1)
	{
		bias_fc4[i] = abias_fc4[i];
		//bias_fc4[i] =6;
		
    }  
    
 	mean2 = (float*) malloc(16*sizeof(float));
   	variance2 = (float*) malloc(16*sizeof(float));
   	affine_r2 = (float*) malloc(16*sizeof(float));
   	affine_c2 = (float*) malloc(16*sizeof(float));

	for(i=0;i<16;i=i+1)
	{
		mean2[i] = amean2[i];
		variance2[i] = avariance2[i];
		affine_r2[i] = aaffine_r2[i];
		affine_c2[i] = aaffine_c2[i];

    }
	
	
	mean3 = (float*) malloc(8*sizeof(float));
   	variance3 = (float*) malloc(8*sizeof(float));
   	affine_r3 = (float*) malloc(8*sizeof(float));
   	affine_c3 = (float*) malloc(8*sizeof(float));
	...
	{
		mean3[i] = amean3[i];
		variance3[i] = avariance3[i] ;
		affine_r3[i] = aaffine_r3[i];
		affine_c3[i] = aaffine_c3[i];
		//mean3[i] = 7;
		//variance3[i] = 7 ;
		//affine_r3[i] = 7;
		//affine_c3[i] = 77;
    ...
    un_vect2 = (float*) malloc(32*sizeof(float));
	...
	//printf("Hello All declared");



	    un_vect3 = fully_connect(un_vect2,weight_fc2,bias_fc2,16,32);
	    ...
	    free(weight_fc2);
	   	...
	   	un_vect35 = batch_norm(un_vect3,mean2,variance2,affine_r2,affine_c2,16);
	    ...
	    free(affine_c2);
	   	...
	   	un_vect4 = activation_1d(un_vect35,16);

   	
    un_vect5 = fully_connect(un_vect4,weight_fc3,bias_fc3,8,16);
    ...
    un_vect55 = batch_norm(un_vect5,mean3,variance3,affine_r3,affine_c3,8);
    ...
    un_vect6 = activation_1d(un_vect55,8);
    
    un_vect7 = fully_connect(un_vect6,weight_fc4,bias_fc4,4,8);
